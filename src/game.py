...
class Game:
    ...
    def handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT: self.running = False
                ...
                elif event.key == pygame.K_SPACE and self.state == "playing": self.bullets.extend(self.player.shoot())
    ...
    def update(self, dt):
        ...
        if keys[pygame.K_SPACE]: self.bullets.extend(self.player.shoot())
        if self.director.update(dt, self.enemies): self.message, self.message_timer = f"ONDA {self.director.wave}", 1.5
        self.message_timer -= dt; self.combo_timer -= dt
        if self.combo_timer <= 0: self.combo = 0
        for enemy in self.enemies[:]:
            shot = enemy.update(dt, self.player.pos)
            if shot: self.bullets.append(shot)
            if enemy.pos.y > HEIGHT + enemy.radius:
                self.enemies.remove(enemy)
                if self.player.hit(enemy.damage): explosion(self.particles, self.player.pos, RED, 12)
                ...
                for enemy in self.enemies[:]:
                    if bullet.rect.colliderect(enemy.rect):
                        ...
                        if enemy.health <= 0:
                            ...
                            self.score += enemy.score + self.combo * 2; explosion(self.particles, enemy.pos, enemy.color, 22)
                        break
            elif bullet.rect.colliderect(self.player.rect):
                self.bullets.remove(bullet)
                if self.player.hit(bullet.damage): explosion(self.particles, self.player.pos, RED, 13)
        for enemy in self.enemies[:]:
            if enemy.rect.colliderect(self.player.rect):
                self.enemies.remove(enemy)
                if self.player.hit(enemy.damage * 2): explosion(self.particles, enemy.pos, ORANGE, 24)
